    ...
    <div class="col-12 col-md-8 mx-auto card">
        <div class="card-body">
            ...
            <div class="card-header">
                <h5 class="card-title">بروزرسانی آزمون </h5>
            </div>
            <div class="card-body">
                <form method="post" action="{{route('admin.course.exam.update',['Exam'=>$exam->exam])}}" >
                    {{csrf_field()}}
                    {{method_field('PATCH')}}
                    ...
                    <div class="form-group">
                        ...
                        <select id="certificate_id" class="form-control p-0 @error('certificate_id') is-invalid @enderror" name="certificate_id">
                            ...
                            @foreach($certificates as $item)
                                <option value="{{$item->id}}" @if($item->id==old('certificate_id',$exam->certificate_id)) selected @endif >{{$item->certificate}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control @error('pass') is-invalid @enderror" id="pass" name="pass" value="{{old('pass',$exam->pass)}}" />
                        @error('pass')
                        <p class="text text-danger" role="alert">
                            {{ $message }}
                        </p>
